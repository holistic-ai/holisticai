...
class Trainer:
    ...
    def __init__(self, model, dataset, train_args, use_debias):
        ...
        self.steps_per_epoch = (len(dataset) + self.train_args.batch_size - 1) // self.train_args.batch_size
    ...
    def build(self, dataset):
        ...
        self.scheduler_cls = torch.optim.lr_scheduler.LinearLR(self.optimizer_cls, start_factor=0.99, total_iters=20)
    ...
    def _train_step_with_debias(self, data):
        ...
        dloss_adv = torch.autograd.grad(outputs=loss_adv, inputs=classifier_params, retain_graph=True)
        ...
        dloss_adv = torch.autograd.grad(outputs=loss_adv, inputs=adversarial_params, retain_graph=True)
