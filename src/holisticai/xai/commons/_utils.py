...
def select_feature_importance_strategy(learning_task_settings: LearningTaskXAISettings, strategy: Literal["permutation", "surrogate"]) -> callable:
    if strategy == "permutation":
        return PermutationFeatureImportanceCalculator(learning_task_settings=learning_task_settings, random_state=RandomState(42))

    if strategy == "surrogate":
        return SurrogateFeatureImportanceCalculator(learning_task_settings=learning_task_settings, random_state=RandomState(42))
...
def compute_xai_features(dataset, learning_task_settings: LearningTaskXAISettings, feature_importance_fn: callable):
    ...
    conditional_feature_importance = compute_conditional_feature_importance(learning_task=learning_task_settings.learning_task, ds=dataset, feature_importance_fn=feature_importance_fn)
    partial_dependence = compute_partial_dependence(x=dataset['X'], features=feature_importance.feature_names, learning_task_settings=learning_task_settings)
    XAIFeatures = namedtuple("XAIFeatures", ["feature_importance", "ranked_feature_importance", "conditional_feature_importance", "partial_dependence"])  # noqa: PYI024
    return XAIFeatures(
