...
import numpy as np
from holisticai.xai.commons._definitions import ConditionalFeatureImportance, FeatureImportance, Importances
...
def compute_conditional_feature_importance(learning_task, ds: Dataset, feature_importance_fn: callable) -> ConditionalFeatureImportance:
    ds = create_output_groups(ds, learning_task)
    conditional_feature_importance = {
        group_name[0]:feature_importance_fn(ds=group_ds)
...
def create_output_groups(ds, learning_task='regression'):
    ...
    if learning_task in ['regression']:
        ...
        return ds.map(lambda sample: {'group': quantil_classify(v[1], v[2], v[3], labels, sample['y'])}, vectorized=False)
