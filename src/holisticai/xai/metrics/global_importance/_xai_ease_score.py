...
if TYPE_CHECKING:
    from holisticai.xai.commons._definitions import FeatureImportance, PartialDependence
...
class XAIEaseScore(BaseModel):
    ...
    Methods:
        __compute_xai_ease_score_data: Computes the XAI Ease Score data for a given partial dependence plot.
    ...
    def compute_xai_ease_score_data(self, partial_dependence):
        """
    ...
    def xai_feature_ease_score(self, partial_dependence) -> float:
        """
        Computes the XAI Ease Score for a single partial dependence plot.

        Args:
            partial_dependence (DataFrame): The partial dependence plot.
