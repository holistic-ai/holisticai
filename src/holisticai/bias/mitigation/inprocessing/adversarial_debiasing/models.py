...
def train_step(cls_state, adv_state, batch, use_debias, adversary_loss_weight:float, rng):
    ...
    (loss, (loss_cls, loss_adv)), grads = jax.value_and_grad(loss_fn, argnums=(0), has_aux=True)(cls_state.params, adv_state.params, batch, rng)
...
def create_train_state(rng, model, learning_rate, feature_dim):
    ...
    adv_state = train_state.TrainState.create(apply_fn=model.adversarial.apply, params=adversarial_params, tx=adversary_tx)
