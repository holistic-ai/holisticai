...
class GDPoisoner:
    ...
    colmap : dict
        Map of columns for onehot encoded features.
    ...
    def __init__(self, eta, beta, sigma, eps, objective, opty, colmap, poison_proportion, num_inits=1):
    ...
    def poison_data(self, X_train, y_train, return_data=False):
        """
        ...
        y_train : Series, shape (n_samples,)
            The target values.
        return_data : bool
            Whether to return the complete data or just poisoned data.
        Returns
        -------
        """
        ...
        if return_data:
            x_out = np.concatenate((X_train, best_poisx), axis=0)
            y_out = np.concatenate([y_train, best_poisy])
...
class LinRegGDPoisoner(GDPoisoner):
    ...
    def __init__(
        ...
        if colmap is None:
            colmap = {}
        GDPoisoner.__init__(self, eta, beta, sigma, eps, objective, opty, colmap, poison_proportion, num_inits)
        self.initlam = 0
