...
class ZooAttack(BaseModel):
    ...
    def generate(self, x_df: pd.DataFrame, y: Optional[np.ndarray] = None, **kwargs) -> pd.DataFrame:  # noqa: ARG002
        """
    ...
    def _optimizer_adam_coordinate(
        ...
        if proj and hasattr(self, "clip_values") and self.clip_values is not None:
            clip_min, clip_max = self.clip_values
