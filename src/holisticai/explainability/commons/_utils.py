...
import pandas as pd
from holisticai.explainability.commons import PermutationFeatureImportanceCalculator, SurrogateFeatureImportanceCalculator
...
def global_feature_importances(dataset, learning_task_settings: LearningTaskXAISettings, feature_importance_calculator: callable):
    ...
    conditional_feature_importance = compute_conditional_feature_importance(
        learning_task=learning_task_settings.learning_task, ds=dataset, feature_importance_calculator=feature_importance_calculator
    )
    return feature_importance,conditional_feature_importance


def local_feature_importances(dataset, learning_task_settings: LearningTaskXAISettings, feature_importance_calculator: callable):
    local_conditional_feature_importances = compute_conditional_feature_importance(
        learning_task=learning_task_settings.learning_task, ds=dataset, feature_importance_calculator=feature_importance_calculator
    )
    feature_importances = pd.concat([cfi.feature_importances for cfi in local_conditional_feature_importances.conditional_feature_importance.values()], axis=0)
...
def compute_global_from_local(local_feature_importance: LocalImportances, local_conditional_feature_importance: LocalConditionalFeatureImportance):
    ...
    conditional_feature_importance = ConditionalFeatureImportance(conditional_feature_importance = {group_name: local_to_global(lfi=lcfi)
                                        for group_name,lcfi in local_conditional_feature_importance.conditional_feature_importance.items()})
    return feature_importance, conditional_feature_importance

def compute_explainability_features(dataset, learning_task_settings: LearningTaskXAISettings, strategy: Union[Literal["permutation", "surrogate"],callable] = "permutation"):
    ...
    if feature_importance_calculator.importance_type == "global":
        feature_importance, conditional_feature_importance = global_feature_importances(dataset=dataset,
        ...
        local_feature_importance, local_conditional_feature_importance = local_feature_importances(dataset=dataset,
                                                                                    learning_task_settings=learning_task_settings,
                                                                                    feature_importance_calculator=feature_importance_calculator)
        feature_importance, conditional_feature_importance =  compute_global_from_local(local_feature_importance, local_conditional_feature_importance)
    else:
    ...
    XAIFeatures = namedtuple(  # noqa: PYI024
        "XAIFeatures",
        ["feature_importance", "ranked_feature_importance", "conditional_feature_importance", "partial_dependence", "local_feature_importance", "local_conditional_feature_importance"],
    )
    return XAIFeatures(
