...
if TYPE_CHECKING:
    import pandas as pd
    from holisticai.explainability.commons._definitions import LearningTaskXAISettings
...
def compute_partial_dependence(x: pd.DataFrame, features: list[str], learning_task_settings: LearningTaskXAISettings):
    ...
    if learning_task_settings.learning_task in ["binary_classification", "multi_classification"]:
        nb_classes = len(learning_task_settings.classes)
