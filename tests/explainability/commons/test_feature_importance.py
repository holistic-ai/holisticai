...
from holisticai.explainability.commons._definitions import Importances
...
def test_compute_ranked_feature_importance():
    ...
    ranked_importances = compute_ranked_feature_importance(feature_importance)
    ...
    ranked_importances = compute_ranked_feature_importance(feature_importance, alpha=0.8)
    assert np.isclose(ranked_importances.values, np.array([0.6, 0.15])).all()
    assert ranked_importances.feature_names == ['feature1', 'feature2']

    # Test filtering with custom threshold (alpha=0.5)
    ranked_importances = compute_ranked_feature_importance(feature_importance, alpha=0.5)
    ...
    ranked_importances = compute_ranked_feature_importance(feature_importance, alpha=0.2)
    assert np.isclose(ranked_importances.values , np.array([0.6])).all()
    assert ranked_importances.feature_names == ['feature1']
