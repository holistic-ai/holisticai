...
from holisticai.xai.commons import SurrogateFeatureImportance, PermutationFeatureImportance
from holisticai.xai.commons._definitions import SFeatureImportance, PFeatureImportance
...
def test_surrogate_feature_importance_call(input_data):
    ...
    fi_strategy = SurrogateFeatureImportance(learning_task_settings=learning_task_settings, random_state=RandomState(42))
    result = fi_strategy(test)
    assert isinstance(result, SFeatureImportance)
...
def test_permutation_feature_importance_call(input_data):
    ...
    from holisticai.xai.commons._definitions import BinaryClassificationXAISettings
    learning_task_settings = BinaryClassificationXAISettings(predict_fn=model.predict, predict_proba_fn=model.predict_proba, classes=[0,1])
    fi_strategy = PermutationFeatureImportance(learning_task_settings=learning_task_settings, random_state=RandomState(42))
    result = fi_strategy(test)
    assert isinstance(result, PFeatureImportance)
