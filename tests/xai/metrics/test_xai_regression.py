...
from holisticai.xai.metrics import regression_xai_metrics, regression_xai_features
...
def test_xai_regression_metrics(input_data):
    ...
    metrics = regression_xai_metrics(test['X'], test['y'], model.predict)
...
def test_xai_classification_metrics_separated(input_data):
    ...
    xai_features = regression_xai_features(test["X"], test["y"], model.predict)
