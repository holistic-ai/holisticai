...
def get_protected_values(df, protected_attribute, protected_value):
    ...
    return np.squeeze(group.values)
...
def preprocess_adult_dataset(df, protected_attribute, output_variable, drop_columns):
    ...
    print("Setting 'Female' as 'group a' and 'Male' as 'group b'")
    # Get unique values from 'class' column
    unique_values = df[output_variable].unique()
    # Map values from 'class' column to 0 and 1 accoring to unique values
    output = df[output_variable].map({unique_values[0]: 0, unique_values[1]: 1})
    print(
        f"Mapping '{output_variable}' column, {unique_values[0]} to 0 and {unique_values[1]} to 1"
    )
    # Removing unnecessary columns
    df = df.drop(drop_columns, axis=1)
    print("Removing unnecessary columns")
    # Convert categorical columns to one-hot encoding, except 'class' column
    df = pd.get_dummies(df, columns=df.columns[df.dtypes == "category"])
    # Add 'class' column to the end of the dataframe
...
def preprocess_us_crime_dataset(df, protected_attribute, threshold=0.5):
    ...
    print(f"Setting '{protected_attribute}'>{threshold} as 'group a'")
    group_b = ~group_a
    xor_groups = group_a ^ group_b
...
def load_dataset(dataset="adult", preprocessing=True, small=False):
    if dataset == "adult":
        data = load_adult()
        ...
        if preprocessing:
            ...
            if small:
                df = pd.concat(
                    [
                        df[
                            (df[protected_attribute] == "Male")
                            & (df[output_variable] == ">50K")
                        ]
                        .sample(50)
                        .reset_index(drop=True),
                        df[
            ...
            # Maintain only the columns with less than 1000 nan elements
            df = df.iloc[
                :, [i for i, n in enumerate(df.isna().sum(axis=0).T.values) if n < 1000]
            ]
            return preprocess_adult_dataset(
        ...
        if preprocessing:
            ...
            if small:
                ...
                b_index = list(np.where(group_b == 1)[0][::5])
                indexes = a_index + b_index
                # select indexes from df
                df = df.iloc[indexes]
                # select indexes from group_a and group_b
                group_a = group_a.iloc[indexes]
                group_b = group_b.iloc[indexes]
                return post_process_dataframe(df, group_a, group_b)
            df, group_a, group_b = preprocess_us_crime_dataset(
                df, protected_feature="racePctWhite"
            )
            return post_process_dataframe(df, group_a, group_b)
        return data
    elif dataset == "heart":
        ...
        if preprocessing:
            ...
            if small:
                df[output_variable] = convert_float_to_categorical(
                    df[output_variable], 3
                )
                scalar = StandardScaler()
