...
def test_clustering_efficacy_metrics():
    ...
    X, y = make_blobs(
        n_samples=100, n_features=2, centers=2, cluster_std=0.01, random_state=0
    )
...
def test_multiclassification_efficacy_metrics():
    ...
    metrics = multiclassification_efficacy_metrics(
        y_true, y_pred, classes=["Low", "Medium", "High"], by_class=True
    )
