...
def test_gdbridge_complete(regression_dataset):
    train, test = regression_dataset
    X_train = train['X'].drop(columns=['fold'])
    X_test = test['X'].drop(columns=['fold'])
    ...
    poiser = RidgeGDPoisoner(poison_proportion=0.2, num_inits=1, max_iter=2) #  
...
def test_gdbridge_only_poison(regression_dataset):
    train, test = regression_dataset
    X_train = train['X'].drop(columns=['fold'])
    X_test = test['X'].drop(columns=['fold'])
