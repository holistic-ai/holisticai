...
class Anonymize:
    ...
    def __init__(
        ...
        if k < 2:
            raise ValueError(
                "k should be a positive integer with a value of 2 or higher"
            )
    ...
    def anonymize(self, X_train, y_train):
        ...
        if self.categorical_features and not set(self.categorical_features).issubset(
            set(self.features_names)
        ):
            raise ValueError(
        ...
        self.quasi_identifiers = [
            i for i, v in enumerate(self.features_names) if v in self.quasi_identifiers
        ]
        ...
        if self.categorical_features:
            self.categorical_features = [
                i
                for i, v in enumerate(self.features_names)
                if v in self.categorical_features
            ]
    ...
    def _anonymize(self, x, y):
        ...
        if any(x.select_dtypes(include="category").columns):
            if not self.categorical_features:
                raise ValueError(
                    "when supplying an array with non-numeric data, categorical_features must be defined"
                )
        ...
        if self.is_regression:
            self._anonymizer = DecisionTreeRegressor(
                random_state=10, min_samples_split=2, min_samples_leaf=self.k
            )
        else:
            self._anonymizer = DecisionTreeClassifier(
                random_state=10, min_samples_split=2, min_samples_leaf=self.k
            )
    ...
    def _find_representatives(self, x, x_anonymizer_train, cells):
        ...
        for cell in cells:
            ...
            indexes = [
                index for index, node_id in enumerate(node_ids) if node_id == cell["id"]
            ]
            ...
            for (
                feature
            ) in self.quasi_identifiers:  # self.quasi_identifiers are numerical indexes
                if feature not in done:
                    ...
                    if feature in all_one_hot_features:
                        ...
                        if (
                            self.categorical_features
                            and feature in self.categorical_features
                        ):
                            # find most common value
                            cell["representative"][feature] = Counter(
                                values
                            ).most_common(1)[0][0]
                        else:
    ...
    def _find_sample_nodes(self, samples):
        ...
        return [
            (list(set([i for i, v in enumerate(p) if v == 1]) & node_set))[0]
            for p in paths
        ]
    ...
    def _modify_categorical_features(self, x):
        ...
        numeric_features = [
            ...
            if f in used_features and f not in self.categorical_features
        ]
        categorical_features = [
            f for f in self.categorical_features if f in used_features
        ]
