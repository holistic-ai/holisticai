...
def BlackBoxAttack(attack_feature, X_train, y_train, X_test, y_test):
    ...
    categorical_features = append_if_not_empty(
        categorical_features,
        X_train_attack.select_dtypes(include=["category"]).columns.tolist(),
    )
    # Create transformers for numerical and categorical features
    ...
    preprocessor = ColumnTransformer(
        transformers=[
            (
                "num",
                numeric_transformer,
                X_train_attack.select_dtypes(exclude=["category"]).columns,
            ),
    ...
    model = mm.fit(
        train_encoded[:attack_train_size], y_train_attack[:attack_train_size]
    )
    # Predict values
