...
from holisticai.robustness.mitigation.attacks.attack import (
    AttributeInferenceAttack,
    MembershipInferenceAttack,
)
from holisticai.robustness.mitigation.utils.exceptions import EstimatorError
from holisticai.robustness.mitigation.utils.formatting import get_feature_index
from holisticai.robustness.utils import is_estimator_valid
...
class AttributeInferenceMembership(AttributeInferenceAttack):
    ...
    def __init__(
        ...
        if not is_estimator_valid(
            estimator, estimator_requirements=self.estimator_requirements
        ):
            raise EstimatorError(
                membership_attack.__class__,
                membership_attack.estimator_requirements,
                estimator,
            )
    ...
    def infer(
        ...
        if self.estimator.input_shape is not None:
            if (
                ...
                and self.estimator.input_shape[0] != x.shape[1] + 1
            ):
                raise ValueError(
                    "Number of features in x + 1 does not match input_shape of the estimator"
                )
        ...
        if isinstance(self.attack_feature, int):
            ...
            for index, value in enumerate(values):
                ...
                x_value = np.concatenate(
                    (x[:, : self.attack_feature.start], curr_value), axis=1
                )
                x_value = np.concatenate(
                    (x_value, x[:, self.attack_feature.start :]), axis=1
                )
    ...
    def _check_params(self) -> None:
        ...
        if not isinstance(self.membership_attack, MembershipInferenceAttack):
            raise ValueError(
                "membership_attack should be a sub-class of MembershipInferenceAttack"
            )
