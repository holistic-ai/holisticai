...
from holisticai.robustness.mitigation.summary_writer import (
    SummaryWriter,
...
if TYPE_CHECKING:
    from holisticai.wrappers.formatting import CLASSIFIER_TYPE, GENERATOR_TYPE
...
class Attack(abc.ABC):
    ...
    def _check_params(self) -> None:
        ...
        if not isinstance(self._summary_writer_arg, (bool, str, SummaryWriter)):
            raise ValueError(
                "The argument `summary_writer` has to be either of type bool or str."
            )
...
class PoisoningAttack(Attack):
    ...
    @abc.abstractmethod
    def poison(
        self, x: np.ndarray, y=Optional[np.ndarray], **kwargs
...
class InferenceAttack(Attack):
    ...
    def __init__(self, estimator):
        ...
        :type estimator: :class:`.holisticai.wrappers.estimator.BaseEstimator`
        """
