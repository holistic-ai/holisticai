...
def compute_feature_scores(data, threshold):
    scores = [{'feature':feat, 'scores':sum([1 if rr>threshold else 0 for rr in r]),'few_points':flag} \
...
def compute_partial_dependence(model, feature_importance, x, target=None):
    ...
    partial_dependence = partial_dependence_creator(model, grid_resolution = grid_resolution, x=x, feature_ids=feature_importance_indexes, target=target)
    data = {feat : compute_similarity(df, num_chunks) for feat, df in partial_dependence.items()}
    score_data = compute_feature_scores(data, threshold)
    score_data['scores'] = score_data.apply(lambda x: index_to_class[x['scores']], axis=1)
...
def explainability_score(model_type, model, x, target, feature_importance):
    if model_type == 'binary_classification':
        ...
        result = pd.DataFrame.from_dict({'Global Explainability Score': compute_partial_dependence(model, feature_importance, x, target)
                        for target in targets}, orient='index')

    elif model_type == 'regression':
        result = pd.DataFrame.from_dict({'Global Explainability Score':compute_partial_dependence(model, feature_importance, x)}, orient='index')
