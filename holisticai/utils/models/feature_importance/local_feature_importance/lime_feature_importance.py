from holisticai.explainability.metrics.utils import lime_creator, get_index_groups
...
def compute_lime_feature_importance(model_type, model, x, y):
    ...
    features_importance = lime_creator(scorer=scorer, X=x, index_groups=index_groups, mode=lime_mode)
    conditional_features_importance = {str(c):gdf for c, gdf in features_importance.groupby('Sample Group')}
