...
def feature_importance(model, x, y):
    n_repeats=5
    random_state = 42
    feat_imp = permutation_importance(model, x, y, n_repeats=n_repeats, random_state=random_state)
    df_feat_imp = pd.DataFrame({"Variable": x.columns, "Importance": feat_imp['importances_mean'], "Std": feat_imp['importances_std']})
...
def compute_permutation_feature_importance(model_type, model, x, y):
    ...
    conditional_features_importance = {str(label) : feature_importance(model, x.iloc[index] , y.iloc[index]) for label, index in index_groups.items()}

    return PermutationFeatureImportance(model_type, model, x, y, features_importance, conditional_features_importance)