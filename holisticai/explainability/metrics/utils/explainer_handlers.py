...
class LimeTabularHandler:
    ...
    def explain_one_sample(self, x0):
        exp = self.explainer.explain_instance(x0, self.scorer, num_features=len(x0), num_samples=200)
    ...
    def __call__(self, Xsel):
        values = np.stack([self.explain_one_sample(x) for index,x in Xsel.iterrows()], axis=0)
