...
def bar(feat_imp, max_display=None, title=None, figsize=(7, 5), _type='global'):
    ...
    if _type == 'global':
        df_feat_imp = feat_imp.iloc[0:max_features,:].sort_values(by='Importance', ascending=False)
        ...
        df_feat_imp = feat_imp.groupby('Feature Label')['Importance'].mean().sort_values(ascending=False).reset_index()
    ...
    sns.barplot(y=names, x=importance, palette=hai_palette, ax=ax, )
    ax.set_title(title)
    plt.tight_layout()
    plt.show()
