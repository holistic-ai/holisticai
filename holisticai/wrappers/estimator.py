...
if TYPE_CHECKING:
    ...
    from holisticai.robustness.metrics.verification_decisions_trees import Tree
    from holisticai.robustness.mitigation.data_generators import DataGenerator
    from holisticai.robustness.mitigation.defences.postprocessor.postprocessor import (
        Postprocessor,
    )
    from holisticai.robustness.mitigation.defences.preprocessor.preprocessor import (
        Preprocessor,
    )
    from holisticai.robustness.utils import CLIP_VALUES_TYPE, PREPROCESSING_TYPE
...
class BaseEstimator(ABC):
    ...
    def _check_params(self) -> None:
        ...
        if self._clip_values is not None:
            ...
            if np.array(
                self._clip_values[0] >= self._clip_values[1]
            ).any():  # pragma: no cover
            ...
            for preprocess in self.preprocessing_operations:
                if not isinstance(preprocess, Preprocessor):  # pragma: no cover
                    raise ValueError(
                        ...
                        "holisticai.robustness.mitigation.defences.preprocessor.preprocessor.Preprocessor."
                    )
        else:  # pragma: no cover
            raise ValueError(
                "All preprocessing defences have to be instance of "
