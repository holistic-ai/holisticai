...
from holisticai.robustness.mitigation.utils.formatting import to_categorical
from holisticai.wrappers.certification.abstain import AbstainPredictorMixin
...
class NeuralCleanseMixin(AbstainPredictorMixin):
    ...
    def _fit_classifier(
        self, x: np.ndarray, y: np.ndarray, batch_size: int, nb_epochs: int, **kwargs
    ) -> None:
        raise NotImplementedError
    ...
    def predict(
        ...
        **kwargs
        ...
        :return: Array of predictions of shape `(nb_inputs, nb_classes)`.
        """
        predictions = self._predict_classifier(
            x=x, batch_size=batch_size, training_mode=training_mode, **kwargs
        )

        if len(self.top_indices) == 0:
            logger.warning(
                "Filtering mitigation not activated, suspected backdoors may be triggered"
            )
            return predictions
        ...
        predictions[
            np.any(suspected_neuron_activations > self.activation_threshold, axis=1)
        ] = self.abstain()

        return predictions

    def mitigate(
        self, x_val: np.ndarray, y_val: np.ndarray, mitigation_types: List[str]
    ) -> None:
        """
        ...
        """
        clean_data, backdoor_data, backdoor_labels = self.backdoor_examples(
            x_val, y_val
        )
        ...
        if "pruning" in mitigation_types or "filtering" in mitigation_types:
            ...
            backdoor_activations = self._get_penultimate_layer_activations(
                backdoor_data
            )

            # rank activations descending by difference in backdoor and clean inputs
            ranked_indices = np.argsort(
                np.sum(clean_activations - backdoor_activations, axis=0)
            )
        ...
        for mitigation_type in mitigation_types:
            if mitigation_type == "unlearning":
                ...
                self._fit_classifier(
                    backdoor_data, backdoor_labels, batch_size=1, nb_epochs=1
                )
                ...
                backdoor_effective = self.check_backdoor_effective(
                ...
                avg_clean_activation = np.average(
                    clean_activations[:, self.top_indices], axis=0
                )
                std_clean_activation = np.std(
                    clean_activations[:, self.top_indices], axis=0
                )
                ...
                self.activation_threshold = (
                    avg_clean_activation + 1 * std_clean_activation
                )

            else:  # pragma: no cover
                raise TypeError(
                    "Mitigation type: `" + mitigation_type + "` not supported"
                )

    def check_backdoor_effective(
        self, backdoor_data: np.ndarray, backdoor_labels: np.ndarray
    ) -> bool:
        """
        ...
        backdoor_effective = np.logical_not(
            np.all(backdoor_predictions == backdoor_labels, axis=1)
        )
        return np.any(backdoor_effective)  # type: ignore

    def backdoor_examples(
        ...
        for backdoored_label, mask, pattern in self.outlier_detection(x_val, y_val):
            data_for_class = np.copy(
                x_val[np.argmax(y_val, axis=1) == backdoored_label]
            )
            labels_for_class = np.copy(
                y_val[np.argmax(y_val, axis=1) == backdoored_label]
            )

            if len(data_for_class) == 0:
                logger.warning(
                    "No validation data exists for infected class: %s",
                    str(backdoored_label),
                )
    ...
    def outlier_detection(
        ...
        for class_idx in range(num_classes):
            ...
            if l1_norms[class_idx] <= median and anomaly_index > 2:
                logger.warning(
                    "Detected potential backdoor in class: %s", str(class_idx)
                )
