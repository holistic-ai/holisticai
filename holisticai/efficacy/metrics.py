...
def classification_efficacy_metrics(y_pred, y_true=None, y_proba=None):
    ...
    metrics = [
        [pf, fn(y_pred, y_true, **hyper[pf]), ref_vals[pf]]
        for pf, fn in perform.items()
    ]
    if y_proba is not None:
        opp_metrics = [
            [pf, fn(y_pred, y_true, **soft_hyper[pf]), ref_vals[pf]]
            for pf, fn in soft_perform.items()
        ]
        metrics += opp_metrics

    return pd.DataFrame(metrics, columns=["Metric", "Value", "Reference"]).set_index(
        "Metric"
    )
...
def clustering_efficacy_metrics(X, y_pred):
    ...
    metrics = [
        [pf, fn(X, y_pred, **hyper[pf]), ref_vals[pf]] for pf, fn in perform.items()
    ]
...
def multiclassification_efficacy_metrics(
    y_pred, y_true=None, y_proba=None, classes=None, by_class=False
):
    """
    ...
    if classes is None:
        ...
        if y_proba is not None:
            ...
            ), f"to evaluate y_proba outputs, parameter `classes` must be passed."
    ...
    def process_metrics(
        values_list, metrics_dict, y_pred, y_true_oh, n_classes, detailed_hyper
    ):
        metrics_list = []
    ...
    if by_class:
        metrics_1 = process_metrics(
            metrics_1, perform, y_pred, y_true, n_classes, detailed_hyper
        )
        metrics = [
            [pf] + metric + [ref_vals[pf]]
            for metric, (pf, fn) in zip(metrics_1, perform.items())
        ]
    else:
        metrics = [
            [pf] + [metric] + [ref_vals[pf]]
    ...
    if y_proba is not None:
        ...
        if by_class:
            metrics_2 = process_metrics(
                metrics_2, soft_perform, y_proba, y_true_oh, n_classes, detailed_hyper
            )
            metrics_2 = [
                [pf] + metric + [ref_vals[pf]]
                for metric, (pf, fn) in zip(metrics_2, soft_perform.items())
            ]
        else:
            metrics_2 = [
                [pf] + [metric] + [ref_vals[pf]]
    ...
    return pd.DataFrame(
        metrics, columns=["Metric"] + values_cols + ["Reference"]
    ).set_index("Metric")
