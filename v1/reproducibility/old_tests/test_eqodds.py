...
def running_without_pipeline(small_categorical_dataset):
    ...
    post = EqualizedOdds()
    post.fit(train['y'], y_train_pred, group_a=train['group_a'], group_b=train['group_b'])

    X_test_scaled = scaler.transform(test['x'])

    y_pred = model.predict(X_test_scaled)
    y_pred = post.transform(y_pred, group_a=test['group_a'], group_b=test['group_b'])[
        "y_pred"
    ]

    return classification_bias_metrics(test['group_b'], test['group_a'], y_pred, test['y'])
...
def running_with_pipeline(small_categorical_dataset):
    ...
    y_pred = pipeline.predict(
        test['x'], bm__group_a=test['group_a'], bm__group_b=test['group_b']
    )
